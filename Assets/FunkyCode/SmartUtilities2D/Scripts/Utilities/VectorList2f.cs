...
public class VectorList2f {
	...
	static public List<Vector2f> ToWorldSpace(Transform transform, List<Vector2f> pointsList)
	{
		List<Vector2f> resultList = new List<Vector2f>(); 
		foreach (Vector2f id in pointsList)
			resultList.Add (new Vector2f(transform.TransformPoint (id.vector)));
	...
	public static List<Vector2f> GetListStartingIntersectLine(List<Vector2f> pointsList, Pair2f line)
	{
		...
		List<Pair2f> pointsPairList = Pair2f.GetList(pointsList);
		
		foreach (Pair2f p in pointsPairList) {
