...
public class LinearSlicerExtended {
	...
	static public Slice2D PointExplode(Polygon2D polygon, Vector2f point)
	{
		if (polygon.PointInPoly (point) == false)
			return(Slice2D.Create(point));
	...
	{
		Slice2D result = Slice2D.Create(Slice2D.SliceType.Exploding);
	...
	// Slice From Point
	static public Slice2D SliceFromPoint(Polygon2D polygon, Vector2f point, float rotation)
	{
